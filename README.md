...
Browser (http://localhost/)
  ...
  │   Micrometer + OTel Java Agent         │
  └──────┬─────────────────┬───────────────┘
         │  :6379 (内部)    │  OTLP
         ▼                  ▼
  ┌──────────────┐   ┌───────────────────────────────────┐
  │ Redis Alpine │   │  OTel Collector                   │
  │ AOF / 認証   │   │  → Jaeger(:16686)   トレース      │
  └──────────────┘   │  → Prometheus(:9090) メトリクス    │
                     │  → Loki(:3100)       ログ         │
                     └───────────────────────────────────┘
                              ↓
                     ┌────────────────┐
                     �� Grafana(:3000) │  統合ダッシュボード
                     └────────────────┘
```
...
| 技術 | バージョン |
|------|-----------|
| OpenTelemetry Collector | 0.120.0 |
| Prometheus | 3.4.0 (retention: 7d) |
| Grafana | 11.6.0 |
| Jaeger | 2.6.0 (trace永続化済み) |
| Loki | 3.4.2 |
